...
const LinkCard = ({ link, loading = false }) => {
  ...
  return (
    <>
      <div
        className="group relative block rounded-xl p-4 sm:p-6 transition-all duration-300 hover:-translate-y-1 focus-visible:-translate-y-1 hover:shadow-lg focus-visible:shadow-lg bg-white/90 backdrop-blur-sm outline-none ring-primary-400 focus-visible:ring-2"
        style={{
      ...
      >
        <div
          ...
          style={{
            background: `linear-gradient(45deg, ${getColourForCategory(
              link.category
            )}05 0%, ${getColourForCategory(link.category)}15 100%)`,
          }}
        />
        ...
        <div className="relative z-10">
          <div className="flex items-start space-x-3 mb-4">
            <div
              className="flex h-10 w-10 items-center justify-center rounded-lg text-white text-sm font-medium shadow-sm shrink-0"
              style={{
                backgroundColor: `${getColourForCategory(link.category)}`,
              }}
            >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                <div className="flex flex-col items-start justify-between">
                  <h2 className="text-sm font-medium text-text-900 group-hover:text-text-700 transition-colors truncate">
                    {link.title}
                  </h2>
                  <span
                    className="text-[11px] px-2 py-0.5 mt-1 text-white font-heading uppercase rounded-md max-w-[130px] truncate"
                    style={{
                      backgroundColor: `${getColourForCategory(link.category)}`,
                    }}
                    title={link.category}
